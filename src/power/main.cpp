...
std::vector<std::unique_ptr<peekpower::sorter<Iterator>>> contestants() {
	std::vector<std::unique_ptr<peekpower::sorter<Iterator>>> algos;
    algos.push_back(std::make_unique<peekpower::nop<Iterator>>());

#ifndef EXCLUDE_POWERSORTS_WITH_SENTINEL
    algos.push_back(std::make_unique<peekpower::powersort_4way<Iterator,24,peekpower::WILLEM_TUNED>>());
#endif // EXCLUDE_POWERSORTS_WITH_SENTINEL
    algos.push_back(std::make_unique<peekpower::powersort_4way<Iterator,24,peekpower::GENERAL_BY_STAGES_SPLIT>>());

#ifndef EXCLUDE_POWERSORTS_WITH_SENTINEL
    algos.push_back(std::make_unique<peekpower::powersort<Iterator,24,peekpower::COPY_BOTH_WITH_SENTINELS>>());
#endif // EXCLUDE_POWERSORTS_WITH_SENTINEL
    algos.push_back(std::make_unique<peekpower::powersort<Iterator,24,peekpower::COPY_BOTH>>());
    algos.push_back(std::make_unique<peekpower::powersort<Iterator,24,peekpower::COPY_SMALLER>>());

	algos.push_back(std::make_unique<peekpower::std_sort<Iterator>>());
	algos.push_back(std::make_unique<peekpower::std_stable_sort<Iterator>>());
	algos.push_back(std::make_unique<peekpower::top_down_mergesort<Iterator, 24, true>>());


	// Further algorithms and variants

#ifndef EXCLUDE_POWERSORTS_WITH_SENTINEL
    algos.push_back(std::make_unique<peekpower::powersort_4way<Iterator,24,peekpower::WILLEM_VALUES>>());
    algos.push_back(std::make_unique<peekpower::powersort_4way<Iterator,24,peekpower::WILLEM_WITH_INDICES>>());
#endif // EXCLUDE_POWERSORTS_WITH_SENTINEL
    algos.push_back(std::make_unique<peekpower::powersort_4way<Iterator,24,peekpower::GENERAL_BY_STAGES>>());


	algos.push_back(std::make_unique<peekpower::peeksort<Iterator, 24>>());

	algos.push_back(std::make_unique<peekpower::quicksort<Iterator, 24, 128, false>>());
	algos.push_back(std::make_unique<peekpower::quicksort<Iterator, 24, 128, true>>());

	algos.push_back(std::make_unique<peekpower::top_down_mergesort<Iterator, 1, true>>());
	algos.push_back(std::make_unique<peekpower::top_down_mergesort<Iterator, 1, false>>());
	algos.push_back(std::make_unique<peekpower::bottom_up_mergesort<Iterator, 24, true>>());

	algos.push_back(std::make_unique<peekpower::timsort<Iterator>>());
	algos.push_back(std::make_unique<peekpower::trotsort<Iterator, false>>());
	algos.push_back(std::make_unique<peekpower::trotsort<Iterator, true>>());
	algos.push_back(std::make_unique<peekpower::nop<Iterator, true>>());
...
void timeSorts(int reps, std::vector<int> sizes, unsigned long seed, inputs::input_generator<Elem> &inputs,
    ...
    if (peekpower::COUNT_MERGE_COSTS) std::cout << "Counting merge costs." << std::endl;
	if (!csv.is_open()) {
	...
	for (auto &&algo : algos) {
		...
		for (int size : sizes) {
			...
			for (int r = 0; r < reps; ++r) {
				...
				peekpower::totalMergeCosts = 0;
				peekpower::totalBufferCosts = 0;
				...
				if (r != 0) {
					...
					csv << algo->name() << "," << msDiff << "," << size << "," << inputs.name() << "," << r << "," << peekpower::totalMergeCosts << "," << peekpower::totalBufferCosts;
